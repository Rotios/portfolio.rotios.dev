...
* Co-engineered an enterprise-scale, Go-based MCP server for the Internal Developer Platform (IDP), backed by Temporal, enabling developers to agentically design and onboard new applications onto Capital One infrastructure using Claude and Windsurf; the server also installs agent skills encoding enterprise-approved coding standards, enabling developers to continue coding and scaffolding applications aligned with enterprise requirements well beyond initial onboarding.
